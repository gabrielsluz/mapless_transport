...
from research_envs.envs.transportation_pose_capsule_tolerance_conditioned_env import TransportationEnvConfig, TransportationEnv, TransportationMixEnv
...
if __name__ == "__main__":
    ...
    config = TransportationEnvConfig(
        ...
        angle_tolerance_range = (np.pi/36, np.pi/6)
    )
    obs_l_dict = {
        k: obstacle_l_dict[k] 
        for k in [
    ...
    while True:
        ...
        if not action is None:
            ...
            print('Pos tolerance: ', env.cur_env.pos_tolerance)
            print('Angle tolerance: ', np.rad2deg(env.cur_env.angle_tolerance))
            print('World pos tolerance: ', env.cur_env.world.goal_tolerance)
