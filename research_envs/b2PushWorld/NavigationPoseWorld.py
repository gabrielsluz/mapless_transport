...
from research_envs.b2PushWorld.AgentDirectionPose import AgentDirectionPose
...
class NavigationWorld:
    ...
    def distToOrientation(self):
        ...
        angle = self.agent.agent_rigid_body.angle % (2*np.pi)
    ...
    def did_agent_reach_goal(self):
        reached_pos = (self.goal['pos'] - self.agent.agent_rigid_body.position).length < self.goal_tolerance['pos']
    ...
    def reset(self):
        ...
        sampled_pos = self.gen_non_overlapping_position(self.goal_tolerance['pos']+self.agent.agent_radius)
    ...
    def drawToBuffer(self):
        ...
        if self.agent_collided == 0:
            ...
            self.drawArrow(screen, self.agent.GetPositionAsList() , self.agent.agent_rigid_body.angle, 10, (0, 0, 1, 0))
        else:
            self.agent.Draw(self.pixels_per_meter, screen, (1, 0, 1, 0), -1)
            self.drawArrow(screen, self.agent.GetPositionAsList() , self.agent.agent_rigid_body.angle, 10, (1, 0, 1, 0))
        # Draw goal
        self.agent.DrawInPose(
