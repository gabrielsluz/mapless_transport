...
def find_best_action(env):
    global forbidden_polys, sg_candidates, is_valid_sg
    self = env
    ...
    if (self.world.goal - self.world.agent.agent_rigid_body.position).length <= self.world.range_max:
    ...
    for sg in sg_candidates:
        ...
        robot_body = Point(sg).buffer(2*self.world.agent.agent_radius)
