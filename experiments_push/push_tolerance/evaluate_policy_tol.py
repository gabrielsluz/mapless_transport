...
config = TransportationEnvConfig(
    ...
    pos_tolerance_range = (2.0, 2.0),
    pos_tolerance_norm = 4.0,
    angle_tolerance_range = (np.pi/18, np.pi/18),
    angle_tolerance_norm = np.pi/9
)
env = TransportationEnv(config)
model = SAC.load('model_ckp/'+exp_name+'/best_model')
print(model.policy)

capsule_width_l = np.linspace(10, 12, 3)
ep_per_width = 500
