...
res_d = {'obj_goal_dist': [], 'force_length': []}

env.reset()
for _ in tqdm(range(ep_per_width)):
    obs, info = env.reset()
    while True:
        action, _states = model.predict(obs, deterministic=True)

        res_d['obj_goal_dist'].append(
            env.world.object_to_goal_vector().length
        )
        res_d['force_length'].append(
            action[1]
        )
...
df.to_csv('eval_results/' + exp_name + '_action_x_dist.csv', index=False)