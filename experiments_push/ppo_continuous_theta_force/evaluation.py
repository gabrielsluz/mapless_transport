...
initial_distance_l = [5.0, 50.0, 100.0, 200.0, 300.0, 400.0, 500.0]
init_d_id = 0
...
def reset_goal_at_distance_from_object(env, distance):
    ...
    env.world.goal.x = env.world.obj.obj_rigid_body.position.x + distance * math.cos(rand_rad)
    env.world.goal.y = env.world.obj.obj_rigid_body.position.y + distance * math.sin(rand_rad)
...
while True:
    ...
    max_corridor_width = max(
        max_corridor_width, 
        max_distance_object_to_line(env.cur_env.world.obj, start_pos, b2Vec2(env.cur_env.world.goal)))
