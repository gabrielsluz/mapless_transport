...
def adjusted_world_reset(self):
    ...
    self.obj.obj_rigid_body.position = (12.5, 80.0)#(12.5, 12.5)
    self.obj.obj_rigid_body.angle = random.uniform(0, 2*np.pi)
    ...
    self.goal['pos'].x = 78.0
    self.goal['pos'].y = 14.0
...
def render():
    ...
    for i in range(len(sg_candidate_l)):
        ...
        if sg_corridor_width_l[i] < corridor_width:
            cv2.circle(screen, sg, 5, (0, 0, 255), -1)
        else:
    ...
    if final_goal is not None:
        ...
        DrawFirstVerticeInPose(
            self.world.obj, final_goal['pos'], final_goal['angle'], 
            self.world.pixels_per_meter, screen, (36, 21, 0), -1, 10)
...
def drawPretty(self):
    ...
    # Draw the subgoal
    self.obj.DrawInPose(
        self.goal['pos'], self.goal['angle'], self.pixels_per_meter, screen, 
        ((94+255)//2, (192+255)//2, (250+255)//2), 
        -1
    )
    DrawFirstVerticeInPose(
        self.obj, self.goal['pos'], self.goal['angle'], 
    ...
    if final_goal is not None:
        self.obj.DrawInPose(
            final_goal['pos'], final_goal['angle'], self.pixels_per_meter, screen, (151, 207, 100), -1)
        DrawFirstVerticeInPose(
            self.obj, final_goal['pos'], final_goal['angle'], 
            self.pixels_per_meter, screen, (36, 21, 0), -1, 10)
    ...
    # Draw empty circle of 25 m around the agent:
    cv2.circle(screen, screen_pos, int(25*self.pixels_per_meter), (0, 0, 0), 3)
...
save_video(frame_l, 'mapless_videos/inner_'+exp_name+'.mp4')